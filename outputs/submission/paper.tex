...
\begin{abstract}
Existing research on AI's economic impact is bifurcated between micro-level task-substitution studies and macro-level productivity analyses, leaving a gap in understanding how effects cascade across sectors and orders of consequence. This paper contributes a structured reconnaissance of that gap and the exploratory method that produces it. We introduce the Multi-LLM Delphi (a two-round foresight procedure in which seven frontier language models from three geopolitical regions are queried as structured, prompt-standardized panelists) and apply it to AI-driven economic transformation across eight GICS-aligned industry verticals, four adoption stages, and three bounding scenarios, yielding an approximately 114,000-word corpus. Because the models share overlapping pre-training data, they are not statistically independent panelists; we therefore treat cross-model agreement as a measure of \emph{epistemic density} (how widely a prediction is embedded across training corpora) rather than as independent corroboration, and we test the dependence structure directly. Round~1 identifies six high-consensus convergence zones, including labor market bifurcation, infrastructure bottleneck repricing, and disintermediation of intermediary business models. These themes are already present in the transformation literature; the contribution is not their novelty but their systematization into a single cross-sector, three-order cascade. Two robustness analyses, added to address the reliability and dependence of these claims, support the central findings: (i)~an independent re-coding of the full corpus by four separate coders yields substantial agreement among coders (mean pairwise Cohen's $\kappa=0.71$) and reproduces five of the six high-consensus zones under majority vote; (ii)~pairwise agreement between models does not track geopolitical provenance (within-bloc agreement is no higher than cross-bloc) indicating that convergence is not an artifact of shared regional training. A threshold-sensitivity analysis shows that three of the six zones are robust to a stricter 6/7 cut-off. Round~2 controlled feedback shifts nine of eleven themes upward, including the ``apprenticeship crash'' (the prediction that eliminating junior roles severs experiential training pipelines) moving from 1/7 to near-unanimous endorsement; because language models exhibit documented acquiescence tendencies, we interpret this movement as an upper bound on latent consensus rather than as independent agreement. Provenance-linked interpretive differences appear (US-origin models foreground market-driven disruption, Chinese-origin models state-steered transformation) but with only two Chinese and one European model these are reported as exploratory observations requiring larger samples, not established findings. Framed through General Purpose Technology theory and the Multi-Level Perspective, the study contributes a cross-sector transformation map and a scalable, replicable instrument for early-stage foresight, explicitly a hypothesis-generating reconnaissance, not validated forecasting.
\end{abstract}
...
This paper addresses the gap with two interlinked contributions, both of which we frame as early-stage reconnaissance rather than validated forecasting. The \textbf{first contribution is a substantive map}: a comprehensive, cross-sector synthesis of AI-driven economic transformation that traces cascading effects across three orders of consequence. Seven frontier language models (representing three geopolitical regions (US, China, EU) and seven corporate ecosystems) were queried in parallel, each in isolation from the others, about AI transformation across eight GICS-aligned industry verticals, four sequential adoption stages, three bounding scenarios, and three orders of effects. We emphasize at the outset that querying models in isolation does not make them \emph{statistically} independent panelists: frontier models are trained on heavily overlapping corpora, so agreement among them reflects the prevalence of a prediction across shared training data, not independent verification. We therefore interpret convergence as a signal of \emph{epistemic density} and test the dependence structure empirically (Section~\ref{sec:methodological_properties}). The resulting ${\sim}$114,000-word corpus, consolidated through structured comparative content analysis with consensus scoring, identifies six high-consensus convergence zones, alongside contested futures and blind spots that reveal where uncertainty persists. Each convergence zone corresponds to a theme already established in the academic literature; the contribution is not that these predictions are novel but that a single, replicable procedure recovers, systematizes, and integrates them into a coherent three-order cascade. A recurring substantive thread is the ``apprenticeship crash'', the prediction that AI's elimination of junior knowledge-work roles will sever the experiential pipeline through which senior professionals are produced, creating a judgment famine within a decade. Initially identified by only one model, this prediction was endorsed by all seven after controlled feedback; because this movement is confounded by acquiescence (Section~\ref{sec:acquiescence}), we present it as a hypothesis worth investigating rather than an established consensus.

The \textbf{second contribution is methodological}: the Multi-LLM Delphi, a foresight instrument in which multiple frontier AI language models serve as structured, prompt-standardized panelists (queried in isolation but trained on partially overlapping corpora) in a two-round design with controlled feedback \citep{linstone1975delphi, gordon2006rtdelphi}. We are deliberate about what this instrument is and is not: it does not reproduce the accountable, experientially grounded judgment of a human expert panel, and we do not claim it does. Its epistemological foundation treats frontier LLMs as structured knowledge synthesizers whose outputs are recombinative syntheses of academic, industry, and policy text, subject to known biases (training-data composition, RLHF alignment, regional data skews) that become analytically informative when compared across model families (Section~\ref{sec:epistemological_status}). To make the instrument auditable rather than merely plausible, this revision adds three quantitative checks that a narrative synthesis cannot provide, inter-coder reliability, inter-model dependence, and consensus-threshold sensitivity. The method's value lies not in replacing human expertise but in providing rapid, scalable, and reproducible reconnaissance of complex transformation landscapes, whose outputs are hypotheses for subsequent validation.
...
The three-order framework used in this study is grounded in an established analytical tradition. The concept of cascading, unanticipated consequences of purposive action was foundationally articulated by \citet{merton1936unanticipated}. In technology assessment, \citet{hilty2006ict} formalized a three-order typology for analyzing ICT effects: first-order effects (direct, immediate), second-order effects (indirect, enabling changes in processes and structures), and third-order effects (systemic behavioral and structural shifts triggered by widespread adoption). \citet{berkhout2004dematerialising} developed a parallel framework distinguishing direct, indirect, and structural effects of digital technologies. In political science, \citet{hall1993policy} proposed three orders of policy change from instrument adjustment (first) through instrument replacement (second) to paradigm shift (third) a framework with direct parallels to the present study's cascade logic.

Applied to AI-driven economic transformation, the three orders map as follows. First-order effects are immediate and directly visible: task-level automation and productivity gains. Second-order effects are structural consequences that emerge when first-order adoption reaches scale: reorganization of value chains, business models, and labor market structures. Third-order effects are systemic, emerging from the compound interaction of first- and second-order changes: shifts in geopolitical power, regulatory regimes, and social contracts. A historical analogy illustrates the cascade: the automobile's first-order effect was personal mobility; its second-order effects included suburbs, highway infrastructure, and the restructuring of retail; its third-order effects extended to the geopolitics of oil \citep{meadows2008thinking}. Applied to AI, the framework systematically traces effects from task automation through organizational restructuring to geopolitical and social transformation. Figure~\ref{fig:cascade} illustrates this cascade logic.
...
\caption{Multi-LLM Delphi Research Design. Seven frontier language models from three geopolitical regions (US: blue, CN: red, EU: green) are queried in parallel with an identical structured prompt instrument specifying 288 analytical cells. The ${\sim}$114,000-word corpus is processed through a four-step analytical pipeline, yielding convergence zones, contested futures, and blind spots.}
...
The geographic distribution (4 US, 2 Chinese, 1 EU) reflects the current landscape of frontier AI development rather than a balanced design. This asymmetry is a genuine limitation: with two Chinese models and a single European model, any provenance-linked pattern is exploratory, and we report it as such throughout (Section~\ref{sec:geopolitical_divergence}).

A more fundamental caveat concerns independence. Delphi methodology assumes panelists form judgments from separate bases of expertise and experience; frontier language models do not. They are trained on heavily overlapping web-scale corpora, share architectural lineage, and are aligned with broadly similar human-feedback procedures. Querying them in separate sessions therefore secures \emph{procedural} independence (no model sees another's answer) but not \emph{statistical} independence (their priors are correlated by construction). This has a direct consequence for interpretation: agreement across models cannot be read as independent corroboration in the way agreement across unrelated human experts can. We address this in two ways. Conceptually, we reinterpret convergence as \emph{epistemic density}, the degree to which a prediction is embedded across shared training data (Section~\ref{sec:epistemological_status}). Empirically, we test whether the agreement structure carries the fingerprint of shared provenance by comparing within-bloc and cross-bloc pairwise agreement (Section~\ref{sec:methodological_properties}). The 4:2:1 distribution, while unbalanced, is what makes this dependence test possible.
...
\textbf{Inter-coder reliability.} Because the primary coding was directed by a single researcher with AI-assisted structuring, we conducted a reliability check to quantify how far the consensus classifications depend on that single interpretive perspective. Four additional coders independently re-coded the full seven-model corpus against the same codebook, deciding for each of the eleven themes $\times$ seven models (77 cells) whether the model substantively addressed the theme.\footnote{The independent coders were separate frontier language models operating under the codebook's inclusion/exclusion rules, not the models that generated the corpus. Using coders drawn from a different pipeline than the data-generating models also provides a partial check on the circularity concern that arises when the same technology both generates and codes the text. We report this as an automated reliability probe; it does not substitute for multiple trained human coders, which remains the standard and is recommended for confirmatory work.} Agreement among the four independent coders was substantial (mean pairwise Cohen's $\kappa=0.71$, range 0.65--0.83), indicating that the codebook itself is applied reproducibly. Agreement between the original coding and the independent coders was moderate (mean $\kappa=0.44$), locating the original Round~1 coding at the lower-moderate end of the reproducible range and motivating the robustness checks reported in Section~\ref{sec:methodological_properties}. Under a majority vote of the four independent coders, five of the six high-consensus zones reported below are reproduced; the independent coders in aggregate identify \emph{more} convergence than the original coding, not less, so the headline claims are conservative with respect to recoding.

\textbf{Step 3: Consensus Scoring.} Adapted from Delphi consensus measurement \citep{vondergracht2012delphi}, convergence was assessed at the thematic level across models. With a seven-member panel the available thresholds are coarse, and we do not claim the 5/7 cut-off for high consensus is uniquely principled; it corresponds to a simple majority-plus-one. To show that the substantive conclusions do not hinge on this choice, Section~\ref{sec:methodological_properties} reports a threshold-sensitivity analysis across all cut-points from 4/7 to 7/7. The classification bands are:
...
\caption{Model Agreement Matrix: Which LLMs Predicted Which Themes (Round~1). Each cell indicates whether a model substantively predicted a theme (filled circle) or did not substantively address it (empty circle); the coding is the binary substantive-vs-not classification of the authoritative Round~1 consensus matrix. Models are grouped by geopolitical origin (US-Aligned, China-Aligned, EU), and the consensus gradient runs from unanimous agreement (top) through moderate consensus to blind spots (bottom). The per-theme scores in this figure match Table~\ref{tab:consensus_shift} and the main text exactly. (This corrects the originally submitted version of the figure, in which three rows (infrastructure bottleneck repricing, concentration via AI feedback loops, and biological-digital convergence) were rendered with cell counts that did not match the underlying consensus matrix.)}
\label{fig:consensus}
\end{figure}
...
Important caveats limit generalizability, and one is decisive enough to bear on how the entire section should be read. With $n$=2 (Chinese) and $n$=1 (European), ``geopolitical origin'' is confounded with the individual model, the developing company, and that company's alignment choices: the single European ``perspective'' is simply Mistral Large~3, and any pattern we attribute to European regulatory culture is indistinguishable, at this sample size, from an idiosyncrasy of one model or one firm's RLHF policy. We therefore cannot claim to have measured a \emph{geopolitical} effect as opposed to a company-level or model-level one. Two further confounds compound this: safety and alignment training may suppress particular claims for reasons unrelated to knowledge (Chinese models may avoid certain geopolitical framings by policy), and training-corpus language mix may drive the US models' market emphasis (their data is dominated by English-language business text). A complementary structural check cuts against the strongest version of the geopolitical reading: when we measure \emph{which} themes models agree on (Section~\ref{sec:methodological_properties}), within-bloc agreement is no higher than cross-bloc, so provenance does not organize the coding-level agreement structure even though it appears to shape interpretive emphasis. Taken together, these patterns should be read as hypotheses motivating a larger, provenance-balanced study with multiple models per region (the minimum design needed to separate region from company from model) not as evidence of a geopolitical partition of AI foresight.


\subsection{Methodological Properties of the Multi-LLM Delphi}
\label{sec:methodological_properties}

Two descriptive properties deserve emphasis. First, the 14:1 ratio between the longest (Claude, 30,613 words) and shortest (Grok, 2,169 words) response reveals substantial variation in output length under identical prompts, introducing weighting effects that future designs should address. Second, models vary systematically in specificity: DeepSeek provides the most detailed causal chains, GPT-5.4 anchors in verifiable data, Gemini produces provocative MAX scenarios, and Mistral produces the most generic output. Only four models (Claude, DeepSeek, GPT-5.4, Gemini) produced complete output for all eight verticals.

Beyond these descriptive properties, three quantitative analyses (added in revision) assess whether the high-consensus findings are robust to coding subjectivity, threshold choice, and the non-independence of the panel. They speak directly to the interpretability of the consensus scores and are summarized in Figures~\ref{fig:reliability}--\ref{fig:threshold}.

\textbf{Inter-coder reliability and classification stability.} As reported in Section~\ref{sec:analytical_procedure}, four independent coders re-coded the full corpus against the study codebook. The coders agree substantially with one another (mean pairwise $\kappa=0.71$) but only moderately with the original coding (mean $\kappa=0.44$), shown in Figure~\ref{fig:reliability}a. The consequence for the substantive claims is favorable: under the majority vote of the four independent coders, five of the six high-consensus zones (labor market bifurcation, infrastructure bottleneck repricing, regulatory divergence, disintermediation, and the trust premium) are reproduced, while concentration via AI feedback loops is the single zone that drops to moderate consensus (Figure~\ref{fig:reliability}b). Notably, the independent coders identify \emph{more} high-consensus zones (seven) than the original coding (six), because two originally moderate themes (autonomous economic agents, compute as strategic reserve) rise to the high-consensus band under recoding. The direction of disagreement therefore does not inflate the paper's claims.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_R1_reliability.png}
\caption{Inter-coder reliability and classification stability. (a)~Pairwise Cohen's $\kappa$ among the original coding and four independent re-coders of the 77-cell (11 themes $\times$ 7 models) matrix: independent coders agree substantially with one another ($\kappa=0.65$--$0.83$) while the original coding sits at the lower-moderate edge (mean $\kappa=0.44$). (b)~Number of models substantively predicting each theme under the original coding (filled) versus the four-coder independent majority (open); five of the six high-consensus zones are reproduced above the 5/7 threshold, and recoding shifts themes upward more often than downward.}
\label{fig:reliability}
\end{figure}

\textbf{Threshold sensitivity.} The number of high-consensus zones depends on the 5/7 cut-off. Under a stricter 6/7 criterion, three zones remain (labor market bifurcation, infrastructure bottleneck repricing, regulatory divergence); under the strictest 7/7 criterion, only labor market bifurcation survives; a looser 4/7 criterion admits eight (Figure~\ref{fig:threshold}). The three zones robust to the 6/7 cut-off constitute the study's most defensible convergence claims, and we flag them accordingly. This transparency replaces the implicit assumption that the 5/7 boundary is uniquely meaningful.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{figures/fig_R3_threshold.png}
\caption{Threshold sensitivity of high-consensus classification. Themes reaching the high-consensus band under alternative agreement cut-offs (4/7 to 7/7). The paper's 5/7 threshold yields six zones; three are robust to a stricter 6/7 cut-off, and one (labor market bifurcation) to unanimity.}
\label{fig:threshold}
\end{figure}

\textbf{Inter-model dependence and the geopolitics of agreement.} Because the panel is not statistically independent, we test whether the agreement structure is organized by geopolitical provenance, the pattern one would expect if shared regional training data were driving convergence. It is not (Figure~\ref{fig:dependence}). Mean pairwise agreement within the US bloc (0.62) is essentially identical to mean cross-bloc agreement (0.62), and the highest-agreeing model pair (GPT--Gemini, 0.91) and the lowest (Claude--Grok, 0.27) are \emph{both} within the US bloc. In other words, provenance does not predict which models agree: the strongest and weakest agreements alike occur among same-region models. This argues against the reading that the convergence zones are a mechanical artifact of a shared US-dominated training distribution. We note two limits on this test. First, a permutation test against each model's marginal prediction rate does not reject the null that overall convergence matches independent base rates ($p=0.32$); with only eleven themes and high marginal rates the test is underpowered, so we do not claim convergence \emph{exceeds} chance, only that it is not structured by geopolitical bloc. Second, agreement in \emph{whether} a theme is addressed does not measure agreement in \emph{how} it is interpreted, the interpretive divergences of Section~\ref{sec:geopolitical_divergence} coexist with this structural finding.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_R2_dependence.png}
\caption{Inter-model dependence does not track geopolitical provenance. (a)~Pairwise agreement (fraction of eleven themes with matching substantive coding) between all model pairs; the highest and lowest agreements both occur \emph{within} the US bloc. (b)~Mean pairwise agreement within the US bloc, within the Chinese bloc, and across blocs: shared regional provenance does not raise agreement above the cross-bloc baseline, arguing against convergence as an artifact of shared regional training data.}
...
Round 2 produced asymmetric consensus dynamics (Figure~\ref{fig:alluvial}): all six high-consensus themes were reaffirmed or strengthened, moderate-consensus themes were upgraded, and no model defected from any convergence zone, a pattern consistent with genuine consensus reinforcement but also with LLMs' documented tendency toward acquiescence when presented with majority positions (see Section~\ref{sec:acquiescence}). The most significant shifts occurred among moderate-consensus themes. Autonomous economic agents moved from 4/7 to 7/7, with models converging directionally while preserving disagreement on timing. The ``compute as currency'' theme (4/7) shifted in an instructive way: rather than simple adoption, multiple models independently converged on an improved formulation (``compute as strategic reserve'') arguing that compute ``behaves more like energy capacity or reserve industrial capability than like money.'' This spontaneous collective refinement represents emergent conceptual precision not documented in classical Delphi studies \citep{vondergracht2012delphi}. Biological-digital convergence (3/7) showed the least movement, remaining at 4/7 and resisting convergence pressure, suggesting genuine uncertainty rather than an information deficit.
...
\subsubsection{What the Multi-LLM Delphi Is, and What It Is Not}
\label{sec:validity}

We first state plainly what this method does not do, because the term ``Delphi'' can invite an inference we do not intend. A classical Delphi elicits the judgment of accountable human experts (people with domain experience, professional stakes in being right, and tacit knowledge that never appears in any text corpus) and its validity rests on that expertise. Language models possess none of these properties. They are statistical models of text; they have no experience, bear no consequences for error, and cannot know anything their training data does not encode. The Multi-LLM Delphi therefore does not reproduce the epistemic warrant of a human expert panel, and cross-model agreement is not evidence in the sense that expert consensus is evidence. We retain the ``Delphi'' label for one narrow reason: the method borrows the classical Delphi's \emph{procedural} architecture (independent elicitation, structured anonymized feedback, and iterative revision) not its epistemic claims. A more guarded reading of the entire study is that it is a structured, multi-source synthesis of the predictive discourse already latent in frontier training corpora, organized by a Delphi-inspired protocol.

Understood in those terms, the method has three properties that make it useful as reconnaissance rather than as forecasting. First, its high-consensus zones align with independently supported predictions in the human literature, labor market bifurcation with \citeauthor{acemoglu2019automation}'s (\citeyear{acemoglu2019automation}) framework, infrastructure repricing with IEA projections, and disintermediation with platform economics theory \citep{goldfarb2019digital}, which is what we would expect if the models are faithfully surfacing the prevailing structure of that literature. This alignment is a validity check on the \emph{instrument} (it recovers what is known), not a discovery of new facts. Second, the coding that produces these zones is reproducible: four independent coders recover five of the six zones (Section~\ref{sec:methodological_properties}). Third, the method produces structured, provenance-linked disagreement that generates research questions a single-model query cannot. Its comparative advantage over a conventional literature review is speed, breadth, and the explicit surfacing of divergence; its disadvantage is the absence of any ground truth or accountable judgment. We therefore position it as a hypothesis-generating complement to expert foresight, not a substitute for it.
...
The \textbf{apprenticeship crash} presents a time-sensitive practical implication: the window for policy intervention is narrow, because once junior roles are eliminated at scale, the experiential training pipelines they supported cannot be easily reconstructed. Firms that proactively redesign apprenticeship systems may gain a durable competitive advantage over those that simply strip junior layers for short-term productivity gains.
...
\textbf{Panel size.} Seven models constitute a small panel by Delphi standards. Classical Delphi studies typically use 10-30 panelists, and the methodological literature cautions against drawing strong conclusions from small panels \citep{vondergracht2012delphi}. However, seven frontier models represent the near-entirety of the current frontier AI landscape, and the diversity across corporate ecosystems and geopolitical origins provides analytical use that partially compensates for small panel size.
...
\textbf{Coding independence and reliability.} The primary thematic coding and consensus scoring were directed by a single researcher with AI-assisted structuring support. To quantify how far the results depend on that single perspective, this revision adds an inter-coder reliability analysis (Sections~\ref{sec:analytical_procedure} and~\ref{sec:methodological_properties}): four independent coders re-coded the full corpus against the study codebook. They agree substantially with one another (mean $\kappa=0.71$), confirming that the codebook is applied reproducibly, but only moderately with the original coding (mean $\kappa=0.44$), indicating that the original Round~1 coding sits at the lower-moderate end of the reproducible range. Reassuringly, five of the six high-consensus zones survive majority-vote recoding, and the direction of disagreement adds rather than removes convergence. Two limitations of this check remain. First, the independent coders are themselves language models applying the codebook, not trained human analysts; this provides a scalable reliability probe and a partial check on the circularity of generating and coding text within the same technology, but it does not substitute for multiple human coders, which remains the standard for confirmatory work. Second, moderate original-vs-independent agreement means individual cell assignments should be treated as uncertain; the robust findings are those stable across coders and thresholds (labor market bifurcation, infrastructure bottleneck repricing, regulatory divergence). Future replications should employ multiple trained human coders alongside the automated probe.
...
The analytical materials supporting this study, including the formal codebook with category definitions and decision rules, Round~1 and Round~2 consensus scoring matrices (CSV), worked coding examples, prompt instruments, detailed figure generation specifications, the complete raw model outputs from both rounds (Markdown format), and the inter-coder reliability re-coding and robustness analyses (inter-coder agreement, inter-model dependence, and threshold sensitivity), are available in the supplementary repository at: \url{https://github.com/TobiasBlask/when-the-middle-disappears}.

\section*{CRediT Authorship Contribution Statement}

\textbf{Tobias-Benedikt Blask:} Conceptualization, Methodology, Investigation, Data Curation, Formal Analysis (direction, validation, and final decisions; AI-assisted structuring), Writing, Review \& Editing, Visualization (direction and specification), Project Administration, Supervision.

The author conceived the research question, designed the Multi-LLM Delphi protocol, selected the model panel, constructed the prompt instruments, conducted all data collection (Round~1 and Round~2) through native model interfaces, directed the thematic coding and consensus scoring with AI-assisted structuring support, evaluated and selected the theoretical framing (GPT theory, MLP integration) identified through AI-assisted literature search, and made all final analytical judgments and interpretive decisions. The author orchestrated the entire manuscript production process using the Open Academic Paper Machine \citep{blask2026openpapermachine}, directing all drafting, reviewing and revising all AI-generated prose, and approving the final manuscript. Writing, Original Draft was performed by the AI tool under author direction and is therefore not claimed as a personal CRediT contribution. AI tools were used as described below.
...
\textbf{Level~2, AI as writing and production tool.} The author used the Open Academic Paper Machine \citep{blask2026openpapermachine}, an autonomous LLM-based research plugin built on Claude Code and the Model Context Protocol, as the primary drafting and production tool for this manuscript. Specifically, the Open Paper Machine was used for:
\begin{itemize}
 \item \textbf{Literature search, synthesis, and reference management:} The AI tool performed automated retrieval, screening, synthesis, and BibTeX compilation of academic references. The literature review sections of this paper (Sections~1--2) were produced through AI-assisted search across academic databases; the AI tool identified relevant sources, synthesized their contributions, and drafted the narrative review. The author reviewed the resulting synthesis for relevance, coherence, and analytical fit, verified key claims against source abstracts, and made all final inclusion and framing decisions, a supervisory role analogous to a senior researcher directing a research team's literature work.
...
\end{itemize}
The tool is openly available at \url{https://github.com/TobiasBlask/open-paper-machine}. A detailed methodological and epistemological discussion of the implications of AI-assisted academic production is provided in \citet{blask2026orchestrator}.
...
\textbf{The author's role, orchestrator and moderator.} The author's contribution lies in the intellectual direction and quality control of this work. The author conceived the research question, designed the methodology, evaluated and selected the theoretical framing proposed by the AI tool through automated literature search, conducted all data collection through native model interfaces, directed the thematic coding and consensus scoring process, made all final interpretive judgments, directed the drafting of every section, reviewed and revised all AI-generated prose, and approved the final manuscript. AI tools assisted with structuring the analytical process, including initial theme extraction and consensus matrix construction, but all final analytical decisions (what counts as a theme, how consensus is scored, which patterns are highlighted) were made by the author. The AI functioned as a research assistant executing tasks under author direction; the author functioned as orchestrator and moderator of an AI-assisted research workflow.

The author takes responsibility for the content of this publication.
...
\vspace{0.5em}
\noindent\small HC = High Consensus (5+ of 7); MC = Moderate Consensus (3--4 of 7). Consensus scores decrease from Stage~1 to Stage~4, consistent with GPT theory's prediction that higher-order, longer-horizon effects are inherently harder to anticipate. The full matrix is available at \url{https://github.com/TobiasBlask/when-the-middle-disappears}.
\end{table}

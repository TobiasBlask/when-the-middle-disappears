...
\caption{Model Agreement Matrix --- Which LLMs Predicted Which Themes (Round~1). Each cell indicates whether a model substantively predicted a theme (filled circle) or did not address it (empty circle). Models are grouped by geopolitical origin (US-Aligned, China-Aligned, EU). The consensus gradient from unanimous agreement (top) to blind spots (bottom) is immediately visible, with per-theme scores in the rightmost column.}
\label{fig:consensus}
\end{figure}
...
Third, the \textbf{organizational middle} compresses as AI flattens hierarchies by automating the coordination, reporting, and information-relay functions that historically justified middle management layers, a dynamic linked to the erosion of default trust in automated judgment (convergence zone 5, trust premium, 5/7 consensus).
